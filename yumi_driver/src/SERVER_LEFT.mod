MODULE SERVER_LEFT
    ...
    CONST bool LOG_LEFT:=FALSE;

    !/////////////////////////////////////////////////////////////////////////////////////////////////////////
    !LOCAL METHODS
    !/////////////////////////////////////////////////////////////////////////////////////////////////////////

    PROC LogTP_LEFT(string msg)
        IF LOG_LEFT=TRUE THEN
            TPWrite msg;
        ENDIF
    ENDPROC
    ...
    PROC ServerCreateAndConnect(string ip,num port)
        ...
        LogTP_LEFT "SERVER: Server waiting for incoming connections ...";
        ...
        WHILE SocketGetStatus(clientSocket)<>SOCKET_CONNECTED DO
            ...
            IF SocketGetStatus(clientSocket)<>SOCKET_CONNECTED THEN
                LogTP_LEFT "SERVER: Problem serving an incoming connection.";
                LogTP_LEFT "SERVER: Try reconnecting.";
            ENDIF
            !//Wait 0.5 seconds for the next reconnection
            WaitTime 0.5;
        ENDWHILE
        LogTP_LEFT "SERVER: Connected to IP "+clientIP;
    ENDPROC
    ...
    PROC main()
        ...
        WHILE TRUE DO
            ...
            addString:="";
            !//Wait for a command
            LogTP_LEFT "waiting for connection";
            SocketReceive clientSocket\Str:=receivedString\Time:=WAIT_MAX;
            LogTP_LEFT "received connection: "+receivedString;
            ...
            CASE 0:
                ...
                IF nParams=0 THEN
                    ok:=SERVER_OK;
                    LogTP_LEFT "Case 0, GoodPing";
                ELSE
                    ok:=SERVER_BAD_MSG;
                    LogTP_LEFT "Case 0, Bad :(";
                ENDIF
                !---------------------------------------------------------------------------------------------------------------
            CASE 1:
                !Cartesian Move
                LogTP_LEFT "Case 1, Good_start";
                IF nParams=7 THEN
                    LogTP_LEFT "Case 1, CORRECT PARAMS";
                    ...
                    LogTP_LEFT "Took: "+addString+"s";
                    moveCompleted:=TRUE;
                    LogTP_LEFT "Case 1, Good";
                ELSE
                    ok:=SERVER_BAD_MSG;
                    LogTP_LEFT "Case 1, Bad";
                ENDIF
            ...
            CASE 2:
                ...
                IF nParams=7 THEN
                    ...
                    LogTP_LEFT "Took: "+addString+"s";
                    moveCompleted:=TRUE;
                    LogTP_LEFT "Case 2, Good_finished";
                ELSE
                    ok:=SERVER_BAD_MSG;
                    LogTP_LEFT "Case 2, Bad";
                ENDIF
                !---------------------------------------------------------------------------------------------------------------
            CASE 3:
                ...
                IF nParams=0 THEN
                    ...
                    !End of string	
                    ok:=SERVER_OK;
                    LogTP_LEFT "Case 3, Good";
                ELSE
                    ok:=SERVER_BAD_MSG;
                    LogTP_LEFT "Case 3, Bad";
                ENDIF
                !---------------------------------------------------------------------------------------------------------------
            CASE 4:
                ...
                IF nParams=0 THEN
                    ...
                    !End of string
                    ok:=SERVER_OK;
                    LogTP_LEFT "Case 4, Good";
                ELSE
                    ok:=SERVER_BAD_MSG;
                    LogTP_LEFT "Case 4, Bad";
                ENDIF
                !---------------------------------------------------------------------------------------------------------------
            CASE 5:
                !Get external axis positions
                IF nParams=0 THEN
                    LogTP_LEFT "Got to Case 5 Good";
                    jointsPose:=CJointT();
            ...
            CASE 11:
                ...
                IF nParams=7 THEN
                    ...
                    moveCompleted:=TRUE;
                    LogTP_LEFT "Case11, Good";
                ELSE
                    ok:=SERVER_BAD_MSG;
                    LogTP_LEFT "Case 11, Bad";
                ENDIF
                !---------------------------------------------------------------------------------------------------------------
            CASE 12:
                ...
                IF nParams=7 THEN
                    ...
                    moveCompleted:=TRUE;
                    LogTP_LEFT "Case 12, Good_finished";
                ELSE
                    ok:=SERVER_BAD_MSG;
                    LogTP_LEFT "Case 12, Bad";
                ENDIF
                !---------------------------------------------------------------------------------------------------------------
            ...
            CASE 99:
                !Close Connection
                IF nParams=0 THEN
                    LogTP_LEFT "SERVER: Client has closed connection.";
                    connected:=FALSE;
                    !//Closing the server
            ...
            CASE 100:
                ...
                !---------------------------------------------------------------------------------------------------------------
            DEFAULT:
                LogTP_LEFT "SERVER: Illegal instruction code";
                ok:=SERVER_BAD_MSG;
            ENDTEST
            !---------------------------------------------------------------------------------------------------------------
    ...
    ERROR (LONG_JMP_ALL_ERR)
        LogTP_LEFT "SERVER: Error Handler:"+NumtoStr(ERRNO,0)+" "+NumToStr(ERR_SOCK_CLOSED,0)+" "+NumtoStr(ERR_SOCK_TIMEOUT,0);
        TEST ERRNO
        CASE ERR_SOCK_CLOSED:
            LogTP_LEFT "SERVER: Lost connection to the client, close&restart socket";
            connected:=FALSE;
            !//Closing the server
            SocketClose clientSocket;
            SocketClose serverSocket;
        ...
        CASE ERR_HAND_NOTCALIBRATED: ! Gripper not calibrated.
            ...
            RETRY;
        DEFAULT:
            LogTP_LEFT "SERVER: Unknown error, close&restart socket";
            connected:=FALSE;
            !//Closing the server
            SocketClose clientSocket;
            SocketClose serverSocket;
