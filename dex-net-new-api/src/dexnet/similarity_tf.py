...
class SimilarityTransform3D:
    ...
    def dot(self, other_tf):
        ...
        if other_tf.to_frame != self.from_frame:
            raise ValueError('Output reference frame of right hand side (%s) must match input reference frame of left hand side (%s)' %(other_tf.to_frame, self.from_frame))
        pose_tf = self.pose.matrix.dot(other_tf.pose.matrix)
    ...
    def __str__(self):
        return str(self.translation.tolist()) + " " + str(self.quaternion.tolist())